...
#         time = datetime.datetime(2016,5,20)
    ...
    def getGreatOrSuper(self,user_id_):
        ...
        normal = CommissionOrder.objects.exclude(**arg).filter(**args).filter(commission_type = CommissionDetail(commission_type = '0')).count()
        great = CommissionOrder.objects.exclude(**arg).filter(**args).filter(commission_type = CommissionDetail(commission_type = '1')).count()
        super = CommissionOrder.objects.exclude(**arg).filter(**args).filter(commission_type = CommissionDetail(commission_type = '2')).count()
